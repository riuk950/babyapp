  ...
  core/
    domain/                                          → LÓGICA PURA (sin framework)                · §3A
      breastmilk/
        ...
        breastmilk_content.dart                      → contenido 4/4 + contentFor(Section?) y disclaimer · RF-1..RF-3, RF-5, CL-7
  features/breastmilk/
    presentation/
...
test/
  core/domain/breastmilk/breastmilk_section_test.dart   · RF-1, CL-1, CL-8
  core/domain/breastmilk/breastmilk_content_test.dart   · RF-1, RF-2, RF-3, RF-5, CL-7
  features/breastmilk/presentation/breastmilk_controller_test.dart · RF-1, RF-4, CL-2, CL-8
  features/breastmilk/presentation/breastmilk_page_test.dart       · RF-1..RF-5, RNF-2, RNF-5, §6
```
...
`main.dart` (igual que en 002). La navegación final entre las funciones no
...
Cuatro modelos en `core/domain/breastmilk/`:
...
4. **Capa Data ausente, justificada** · §2, §3. Descartado: repository genérico vacío innecesario para contenido estático sin I/O.
...
`test/core/domain/breastmilk/breastmilk_section_test.dart` · RF-1, CL-1, CL-8: las 4 secciones con sus etiquetas; orden estable; sin valores inválidos posibles (enum).

`test/core/domain/breastmilk/breastmilk_content_test.dart` · RF-1, RF-2, RF-3, RF-5, CL-7: 4/4 (bestPractices no vacías en todas; highlights y storageTimeRows vacíos solo en Extracción, regla RF-3); disclaimer presente siempre (constante única); determinismo; tabla con 5 filas única en Almacenamiento y sin valores "aceptables"; inventario de términos cerrado (BPA con aclaración embebida); sin sección → `null`.

`test/features/breastmilk/presentation/breastmilk_controller_test.dart` · RF-1, RF-4, CL-2, CL-8, RNF-3: sin sección → contenido `null` con aviso médico; elegir → contenido; cambiar → sustituye; misma sección dos veces → idempotente; reinicio → `null`.

`test/features/breastmilk/presentation/breastmilk_page_test.dart` (widget) · RF-1..RF-5, RNF-2, RNF-5, §6: lista con 4 secciones; contenido tras elegir; bloque destacado con prefijo "Aviso de seguridad" y etiqueta semántica sin sustituir el informativo; aviso médico siempre visible (con y sin sección); scroll de contenido largo; tabla refluyendo a lista a ≤320 dp; retorno con atrás → lista sin sección; etiquetas vía l10n en español.
...
| RF | Módulo | Tests |
|---|---|---|
| RF-1 | `core/domain/breastmilk/breastmilk_section.dart` + `breastmilk_content.dart` + controller | section + content + controller + page |
| RF-2 | `core/domain/breastmilk/breastmilk_guide.dart` + `breastmilk_content.dart` + page | content + page |
| RF-3 | `core/domain/breastmilk/breastmilk_content.dart` + page | content + page |
| RF-4 | `features/breastmilk/presentation/breastmilk_controller.dart` + page | controller + page |
| RF-5 | `core/domain/breastmilk/breastmilk_content.dart` (pureza) | content |
