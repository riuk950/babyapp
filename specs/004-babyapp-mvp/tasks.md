...
- [x] **T-1** Definir `DevelopmentArea` (enum con las 5 áreas y etiquetas en
...
- [x] **T-2** Definir `MilestoneMonth` (enum de 60 meses con `number` y
...
- [x] **T-3** Definir modelos tipados `AreaMilestones`, `MonthContent` y
...
- [x] **T-4** Implementar `milestone_content.dart`: contenido 60/60 y
...
- [x] **T-5** Implementar `last_hit_resolver.dart`: para cada área, si hay hito
...
- [x] **T-6** Implementar `MilestoneController` en
...
- [x] **T-8** Implementar `MilestonePage` (widget) en
...
- [x] **T-9** Conectar `MilestonePage` como `home` temporal en `main.dart`.
  Cubre: RF-1, RNF-5.
