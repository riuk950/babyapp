...
- [x] **T-2** Definir `RoutineMoment` (enum de 6 momentos del día con etiquetas
...
- [x] **T-3** Definir modelo tipado `RoutineTip` { message, source } en
  `routine_tip.dart`. Cubre: RF-3, RF-4.
  - Hecho cuando: los modelos compilan y `flutter analyze` sin hallazgos.

- [x] **T-4** Definir modelo tipado `RoutineGuide` { moment, ageBand, tips } en
  `routine_guide.dart`. Cubre: RF-3.
  - Hecho cuando: los modelos compilan y `flutter analyze` sin hallazgos.

- [x] **T-5** Implementar `routine_content.dart`: contenido 6×4 y
...
- [x] **T-6** Implementar `RoutineController` en
...
- [x] **T-8** Implementar `RoutinePage` (widget) en
...
- [x] **T-9** Conectar `RoutinePage` como `home` temporal en `main.dart`.
  Cubre: RF-1, RNF-5.
