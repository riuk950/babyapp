...
- [x] **T-2** Definir `AlertLevel` (enum: `urgency` / `scheduled`) con su
...
- [x] **T-3** Definir modelos tipados `AlertSignal`, `AreaAlerts` y `AlertGuide`
...
- [x] **T-4** Implementar `alert_content.dart`: contenido 4/4 y
...
- [x] **T-5** Implementar `AlertController` en `features/alerts/presentation/`
...
- [x] **T-6** Configurar `localizationsDelegates` y `supportedLocales` en
...
- [x] **T-7** Implementar `AlertPage` (widget) en
...
- [x] **T-8** Conectar `AlertPage` como `home` temporal en `main.dart`. Cubre:
  RF-1, RNF-5.
...
- [x] **T-9** Ejecutar `flutter pub get && flutter analyze && flutter test`;
